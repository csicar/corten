...
\begin{frame}{Motivation}
  ...
  \begin{tikzpicture}[xscale=.2, yscale=.2]
    \coordinate (good) at (90:7);
    \coordinate (bad) at (-30:7);
    ...
    \node[anchor=west, align=left] (bad_label) at (16,-5) {$h(\texttt{var}) = h'(\texttt{var})$\\ "Variable unchanged between heaps};
    
    \draw[<-, very thick, cyan!60!black, dashed] (good) to[out=45, in=170] (good_label);
    \draw[<-, very thick, cyan!60!black, dashed] (bad) to[out=-45, in=180] (bad_label);
  \end{tikzpicture}
\end{frame}
...
\begin{frame}
  ...
  \begin{tikzpicture}[x=1pt, y=1pt, remember picture]
    \clip (-230 , -40) rectangle (230 , 190);

    \node[help lines, minimum size=40] (rust_character) at (170, 0) {};
    \node[help lines, minimum size=40] (haskell_character) at (-170, 0) {};
    ...
    \node[draw, xkcd-little, rectangle callout, callout pointer shorten=30, callout absolute pointer={(haskell_character)}] (refinement_types) 
      ...
      {\begin{varwidth}{\linewidth}
          ...
          \begin{itemize}
            \item[(I)] $\Gamma \vdash a : \tau_{param}$
            \item[(II)] $\{ v : \texttt{i32} \mid p(v) \} \preceq \{ w : \texttt{i32} \mid q(w) \}$
          \end{itemize}

          Subtype $\{ \tau | p \} \preceq \{ \tau' | q \}$
          \begin{itemize}
            \item[(I)] $\tau \text{ unifies with } \tau'$
          ...
          \begin{enumerate}
            ...
            \item immutable Reference \texttt{\&v}
              \begin{itemize}
                ...
                \item must give back before ownership transfer
              \end{itemize}
