...
An expression constraint is used to restrict $\alpha$ to (as close to) the value of expression $e$. For instance, typing the expression \code{a + 3} should get the strongest type $\alpha == \alpha + 3$ where $\alpha$ is the logic variable for \code{a}.
...
\begin{itemize}
  ...
  \item \textsc{Case \cref{rule:SS-While}}
    Given $\Gamma \vdash \code{while}(x) \{s\} \rightarrow \Gamma_2$, $\sigma : \Gamma$ show $\exists \Gamma_1, \sigma : \Gamma_1 \wedge \Gamma_1 \vdash \code{if}(x) \{\code{while}(x) \{s\}\}\code{else}\{\code{unit}\} \Rightarrow \Gamma_2$
  ...
  \item \textsc{Case \cref{rule:SS-Sub}}
