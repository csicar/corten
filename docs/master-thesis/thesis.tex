...
$$ \begin{gathered}
  ...
  \inferrule*[left=Var-Deref\rulelabel{Var-Deref}]
    {\Gamma \vdash *x \set{y} \\ \Gamma \vdash y : \tau }
    {\Gamma \vdash *x : \tau }
  %
  \\
...
\Cref{rule:May-Ref} stipulates that 
$$ \begin{gathered}
  \inferrule*[left=May-Ref\rulelabel{May-Ref}]
    {\mu(x) = \alpha \\ \vDash \Phi \to \alpha \doteq \&y_1 \vee \dots \vee \alpha \doteq \&y_n}
    {(\mu, \Phi)\vdash *x \in \{ y_1, \dots,y_n \} }
  %
  \\
\end{gathered}$$

Although the rules might not suggest it, in practise inferring good candidates for sets of reference targets can usually be done by a some syntactic analysis that checks the set of logic variables equal to $\alpha$ for a reference predicate. This is easy because the syntax for predicate refinements is very limited.
...
\end{lemma}
\begin{proof} Rule Induction over $\tuple{s}{\sigma} \leadsto \tuple{s_1}{\sigma_1}$
\begin{itemize}
  ...
  \item \textsc{Case \cref{rule:SS-Seq-Inner}}:
    Given: $\tuple{s_1}{\sigma} \leadsto \tuple{s_1'}{\sigma_1}$, \\
     $\Gamma \vdash s_1 ; s_2 \Rightarrow \Gamma_2$, \\
     $\forall \Gamma_2, \Gamma \vdash s_1 \Rightarrow \Gamma_2 \wedge \sigma : \Gamma \to \exists \Gamma_1, \sigma_1 : \Gamma_1 \wedge \Gamma_1 \vdash s_1' \Rightarrow \Gamma_2$ \\
    show $\exists \Gamma_1, \sigma_1 : \Gamma_1 \wedge \Gamma_1 \vdash s_1' ; s_2  \Rightarrow \Gamma_2$.

    Rule inversion over $\Gamma \vdash s_1 ; s_2 \Rightarrow \Gamma_2$ yields $\Gamma \vdash s_1 \Rightarrow \Gamma_1$, $\Gamma_1 \vdash s_2 \Rightarrow \Gamma_2$
