...
The central idea of Refinement Types is adding predicates to a language's type: For example, a type \code{i32} might be refined with the predicate $v > 0$. The refined type is written as $\left\{ v : \text{i32} \mid v > 0 \right\}$ and represent the set of inhabitants that have the specified base type, but also satisfy the predicate. 
...
\code{fn max(x, y) \{ if x > y \{ x \} else \{ y \}\}} and given the return type $\{ v: \text{i32} \mid x \leq v \wedge y \leq v\}$.
...
\paragraph*{Explicit Nullability} Rust will ensure definite assignment, meaning variables can only be accessed if they contain a value. For verification this means, that there is no need to prove that a variable is defined. If nullability is desired, Rust offers normal sum type \code{Option<T>} type with the variants \code{None} and \code{Some(T)}. 
...
\Cref{tab:unsafe-uses-by-crate} shows the totaled result of our analysis: The total number of unsafe uses in an order of magnitude higher for binaries compared to libraries.
...
Looking at the distribution of unsafe uses in Figure \ref{fig:unsafe-ecdf}, we can see that this is an exception: Most other libraries do not use that much unsafe statements. We can also see that even if an executable crate uses \code{unsafe}, it uses few. Note the difference in x- and y-axis scaling: No executable contains more than 63 occurrences on \code{unsafe} blocks, but more than 10\% of libraries contain more than 63 \code{unsafe} sections.
...
\begin{itemize}
	...
	\item \textbf{Parameters} are considered mutable if either the owned value or the reference is mutable. They take the syntactic form: \code{mut a: i32} or \code{b: \&mut i32}
	\item \textbf{Function Definitions} are considered immutable, if all parameters considered immutable. They take the syntactic form: \code{fn f(mut a: i32, b: \&mut i32) \{ ... \}}
	\item \textbf{Arguments} are parts of a function call and can be arbitrary expression, which makes the tracking hard. We consider arguments mutable if are syntactically passed as a mutable reference (for example \code{\&mut i}) and immutable if they are passed as immutable references or literals.
...
\Cref{fig:mutabillity_percentages} shows the ratio of mutable to immutable items. For each syntactical category, the percentages are relative to the total number of occurrences in the dataset. The grey bar represents data points that cound not be determined to be mutable nor immutable.
...
The type system surfaces in two ways in the language, which can be directly embedded in Rust using two macros. Firstly the macro \code{ty!} can be used in place of a Rust type and adds a predicate to the Rust type that any inhabitant of that type must satisfy. For example, the type \code{ty!\{ v : i32 | v >= 0\}} stipulates that a value of Rust type \code{i32} is positive. The second macro \code{relax\_ctx!\{ ... \}} is used for relaxing the context, which will be explained in the subsection \ref{subsec:atomic-updates}.
...
In the example, \code{b = \&mut a} will have inferred type \code{\{ r : \&mut | r == \& a \}}, meaning any inhabitant of this type can at most refer to the variable \code{a}\footnote{It would also be possible to encode the path condition in the reference type, but this was decided against for the stated goal of simplicity for the user. This why no new construct needs to be introduced}.
...
$$
  \inferrule*[left=Fn-Decl\rulelabel{Fn-Decl}]
    { \mu_{init} = \{ x_i \mapsto \alpha_i, y_j \mapsto \delta_j, arg_j \mapsto \beta_j \}
      ...
      \\ \Gamma' \preceq \Gamma_{init}[\{arg_j \mapsto \gamma_j], \chi_j
    }
...
\begin{example}[Sub-Context Relations]
  To get a idea of how the sub-context relation works, consider the following examples:
  \begin{align*}
