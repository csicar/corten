...
\begin{definition}[Progress]
    If $\Gamma \vdash s$, $\sigma : \Gamma$ and $s \neq \code{unit}$, then there is a $s'$ and $\sigma'$ with $\tuple{s, \sigma} \leadsto \tuple{s'}{\sigma'}$.
\end{definition}

\begin{definition}[Preservation]
  If $\Gamma \vdash s$, $\sigma : \Gamma$ and $\tuple{s}{\sigma} \leadsto \tuple{s'}{\sigma'}$, then $\Gamma \vdash s'$ and $\sigma' : \Gamma$
\end{definition}
