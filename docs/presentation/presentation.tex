...
\begin{frame}[fragile]{Modularity \& Mutation}
  \begin{minted}[autogobble, mathescape]{rust}
    fn clamp(a: &mut ty!{ a1 : i32 | true => a2 | a2 <= b1 }, b: ty!{ b1: i32 }) { 
      ...
      if *a > b { *a = b }
      // $\Gamma_2 = (\{a \mapsto v_1, arg_0 \mapsto {\red v_2}, b \mapsto b_1\}, {\red v_2 \leq b_1} \wedge v_1 \doteq \&arg_0 \wedge \text{true} \wedge \text{true})$
    }
    fn client(...) {
      ...
...
\begin{frame}[fragile]{Notion of Sub-Contexts}
  \begin{minted}[autogobble, mathescape]{rust}
    fn clamp(a: &mut ty!{ a1 : i32 | true => a2 | a2 <= b1 }, b: ty!{ b1: i32 }) { 
      ...
    // $\Gamma_2 = (\{a \mapsto v_1, arg_0 \mapsto { v_2}, b \mapsto b_1\}, { v_2 \leq b_1} \wedge v_1 \doteq \&arg_0) $
    }
  \end{minted}
  \begin{itemize}
    \item Question: Is $\Gamma_2$ a valid end-state for \code{clamp}?
    \item Question: Expected end-state?
    \item $\Rightarrow \Gamma_e = (\set{a \mapsto v_1, arg_0 \mapsto a_2, b \mapsto b_1}, v_1 \doteq \&arg_0 \wedge \text{true} \wedge \text{true} \wedge a_2 \leq b_1)$
    \item $\Rightarrow$ $\Gamma_2$ must be a specialization of $\Gamma_e$. Written as $\Gamma_2 \preceq \Gamma_e$
  \end{itemize}
  ...
  \inferrule*[left=$\preceq$-Ctx]
    {
      \vDash v_1 \doteq \&arg_0 \wedge {\red v_2} \leq b_1 \rightarrow v_2 \leq b_1 \wedge v_1 \doteq \&arg_0
      \\ \dom(\Gamma_e) \subseteq \dom(\Gamma_2)
    }
    { \Gamma_2 \preceq \Gamma_e}
  $$
...
\begin{frame}{Future Work}
  ...
  \end{itemize}
  \note{
    - Variant distinktion, more syntax, nested structures
