...
\begin{prooftree}
  \AxiomC{$\Gamma \vdash x : \text{bool} \Rightarrow \Gamma$}
  \AxiomC{$\Gamma, \mu(x) \doteq \code{true} \vdash e_t : \tau \Rightarrow \Gamma'$}
  \AxiomC{$\Gamma, \mu(x) \doteq \code{false} \vdash e_e : \tau \Rightarrow \Gamma'$}
  \LeftLabel{$\text{IF}$}
  \TrinaryInfC{$\Gamma \vdash \text{if } x \text{ then }e_t\text{ else } e_e : \tau \Rightarrow \Gamma'$}
\end{prooftree}

\begin{prooftree}
  \AxiomC{$\Gamma_I, [c \doteq \code{true}] \vdash s \Rightarrow \Gamma_I' $}
