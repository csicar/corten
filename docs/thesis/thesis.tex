...
In our analysis, we found, that firstly crates.io contains significantly more libraries than binaries\footnote{Libraries and Executables are distinguished by checking if they contain \code{bin} or \code{lib} target or one of the corresponing files according to the cargo naming convention.} (see Figure \ref{fig:crate_types}). And secondly libraries are much more likely to use \code{unsafe} Rust. Table shows the result of my own analysis. Where uses of \code{unsafe} are counted and grouped by crate type (see Table \ref{tab:unsafe-uses-by-crate}).
...
\label{subsubsec:mutable-references}\subsubsection{Mutable References}
...
\label{subsec:path-sensitivify}\subsection{Path Sensitivify}
...
\subsection{Try: Statements $\Gamma \vdash s \Rightarrow \Gamma'$}
...
\subsection{Function Declaration Type Checking}
...
\label{subsec:evaluation-complex-mutable-ref}\subsection{Complex Mutable References}
