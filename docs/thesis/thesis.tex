...
\begin{prooftree}
  ...
  \AxiomC{$\{ \beta : b \mid \beta \doteq \alpha \}$}
  \LeftLabel{$\text{VAR}$}
  \BinaryInfC{$\Gamma \vdash x : \{ \alpha : b \mid \varphi\} \Rightarrow \Gamma$}
\end{prooftree}

\begin{prooftree}
  \AxiomC{$\Gamma \vdash c : \text{bool} \Rightarrow \Gamma_c$}
  \AxiomC{$\Gamma_c, c \vdash c_t : \tau \Rightarrow \Gamma_t$}
  \AxiomC{$\Gamma_c, \neg c \vdash c_e : \tau \Rightarrow \Gamma_t$}
  \LeftLabel{$\text{IF}$}
  \TrinaryInfC{$\Gamma \vdash \text{if } c \text{ then }c_t\text{ else } c_e : \tau \Rightarrow \Gamma_t$}
\end{prooftree}
...
\begin{prooftree}
  \AxiomC{$\Gamma \vdash e : \{ \beta \mid \varphi \} \Rightarrow \Gamma'$}
  \AxiomC{$\text{reference destination of } x \text{ is } y$}
  \LeftLabel{$\text{ASSIGN-REF}$}
